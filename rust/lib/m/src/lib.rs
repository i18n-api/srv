...
pub async fn githubNew(p_id:u64,p_login:impl AsRef<str>,p_token:impl AsRef<str>,p_followers:u64,p_mail_li:impl AsRef<str>,p_x_name:impl AsRef<str>,p_com:impl AsRef<str>)->Result<i8>{
let sql = format!("SELECT githubNew({p_id},?,?,{p_followers},?,?,?)");
  Ok(q1!(sql,p_login.as_ref(),p_token.as_ref(),p_mail_li.as_ref(),p_x_name.as_ref(),p_com.as_ref()))
}

#[macro_export]
macro_rules! githubNew {
($p_id:expr,$p_login:expr,$p_token:expr,$p_followers:expr,$p_mail_li:expr,$p_x_name:expr,$p_com:expr) => {
$crate::githubNew($p_id,$p_login,$p_token,$p_followers,$p_mail_li,$p_x_name,$p_com).await?
};
}
